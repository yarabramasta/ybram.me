  ...
  return (
    <ul className="flex flex-col gap-6 lg:w-full">
      {items.map(({ href, text, external: pop }) => (
        <li
          key={text}
          className="text-dark/60 hover:text-dark dark:text-light/60 dark:hover:text-light duration-300 ease-out active:opacity-100 text overflow-hidden relative"
        >
          <Link
            href={href}
